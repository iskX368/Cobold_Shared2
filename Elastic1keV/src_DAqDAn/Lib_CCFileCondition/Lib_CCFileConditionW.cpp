...
//
////正規表現の部品
//static const tstring tBegl = LR"(^)";//行頭
//static const tstring tEndl = LR"((?:;.*$|$))";//行末　コメントとorする
//static const tstring tSpace = LR"([\s\t]*)";//スペース
//static const tstring tComma = LR"([\s\t]*,[\s\t]*)";//コンマ
//static const tstring tNumber_ = LR"(([+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?))";//実数　サブマッチ用カッコつき
//static const tstring tString_ = LR"((\w+))";//文字列　サブマッチ用カッコつき
//static const tstring tStrings_ = LR"((\w+(?:[\s\t]*,[\s\t]*\w+)*[\s\t]*,?))";//文字列のコンマ並び　サブマッチ用カッコつき
//
//static const tstring tLogic1_NOT = LR"(not)";//論理演算子は全部小文字でないとダメ
//static const tstring tLogic1_ = LR"(()" + tLogic1_NOT + LR"())";		//一項論理演算子　サブマッチ用カッコつき
//
//static const tstring tLogic2_AND = LR"(and)";//論理演算子は全部小文字でないとダメ
//static const tstring tLogic2_OR = LR"(or)";
//static const tstring tLogic2_NAND = LR"(nand)";
//static const tstring tLogic2_XOR = LR"(xor)";
//static const tstring tLogic2_ = LR"(()" + tLogic2_AND + LR"(|)" + tLogic2_OR + LR"(|)" + tLogic2_NAND + LR"(|)" + tLogic2_XOR + LR"())";		//二項論理演算子　サブマッチ用カッコつき
//
//static const tstring tDeclParameter = LR"([Pp][Aa][Rr][Aa][Mm][Em][Tt][Ee][Rr])";//宣言
//static const tstring tDeclCondition = LR"([Cc][Oo][Nn][Dd][Ii][Tt][Ii][Oo][Nn])";//宣言
//static const tstring tDeclCoordinate = LR"([Cc][Oo][Oo][Rr][Dd][Ii][Nn][Aa][Tt][Ee])";//宣言
//
//
////正規表現
...
//			Type(Type_),
//			From(Type_ == LogicType::Window ? _wtof(From_Condition1.c_str()) : 0),
//			To(Type_ == LogicType::Window ? _wtof(To_Condition2.c_str()) : 0),
...
//		else {
//			//wchar_tへ変換
//			std::string nam(NAMEOF_ENUM(static_cast<CCFileCondition_Flags::Flag_Condition>(ConditionFlagValue)));
//			wchar_t wbuf[100];
//			size_t ReturnValue;
...
//
//
//std::unique_ptr<CCFC> CCFC::make(std::initializer_list<tstring> List_CCFilePath, const CppAddress_CCFileCoordinateW::Data& Data) {
//	try {
//
//		//map取り出し
...
//					}
//
//					mp.insert(std::make_pair(mch[3].str(), Contents(mch[2].str(), L"", mch[1].str(), logictype)));
//
//				}
//
//
...
//
//
////std::unique_ptr<CCFileCondition> make_CCFileCondition(std::initializer_list<std::basic_string<TCHAR>> List_CCFilePath, const CppAddress_CCFileCoordinate::Data& Data) {
////	try {
////		
////		//map取り出し
////		const auto& mp_int32 = std::get<0>(Data);
