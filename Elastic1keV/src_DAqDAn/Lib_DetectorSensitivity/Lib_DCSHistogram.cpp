...
static const double gv_d = 83;
static const double gv_del = 0.5;
static const double gv_d_e = 5;
static const double gv_del_e = 0.2;
...
static inline double Pnt2X_Backward(const double dPnt, const double delta) {
	return (dPnt + 1) * delta;
}
//微分
static inline double DiffOf_Pnt2X_Backward(const double dPnt, const double delta) {
	return delta;
}

//ポイントから数値を求める  DimDelta/2ずれた位置を0ポイントとして [0, DimDelta) を始まりの区間とする
static inline double Pnt2X_Central0(const double dPnt, const double delta) {
	return (dPnt + 0.5) * delta;
}
...
template<class class_ConversionToDCS, class class_TheoreticalAtomicDCS_Elastic32>
std::unique_ptr<std::vector<std::vector<double>>> DCSTheoretical<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32>::make_2D_vs_Theta_deg_VarPhi_deg(const ScaleInformation& ScaleInfo_Theta_deg, const ScaleInformation& ScaleInfo_VarPhi_deg) {
	try {
		auto vec = std::make_unique<std::vector<std::vector<double>>>();

		const size_t im = ScaleInfo_Theta_deg.NumIntervals();
		const size_t jm = ScaleInfo_VarPhi_deg.NumIntervals();
...
template<class class_ConversionToDCS, class class_TheoreticalAtomicDCS_Elastic32>
std::unique_ptr<std::vector<std::vector<double>>> DCSTheoretical<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32>::make_2D_vs_R_Phi_deg(const ScaleInformation& ScaleInfo_R, const ScaleInformation& ScaleInfo_Phi_deg) {
	try {
		auto vec = std::make_unique<std::vector<std::vector<double>>>();

		const size_t im = ScaleInfo_R.NumIntervals();
		const size_t jm = ScaleInfo_Phi_deg.NumIntervals();
		...
		double DCS;
		double delVarPhi_degdelPhi_deg;
		for (size_t i = 0; i < im; i++) {
			DCS = DCS1D_vs_R->at(i);
			...
			for (size_t j = 0; j < jm; j++) {
				...
				//差分で修正
				const auto& itvl = ScaleInfo_Phi_deg.Interval(j);
				delVarPhi_degdelPhi_deg = (class_ConversionToDCS::VarPhi_deg(itvl.second) - class_ConversionToDCS::VarPhi_deg(itvl.first)) / (itvl.second - itvl.first);

				v.push_back(DCS * delVarPhi_degdelPhi_deg);
			}
...
std::unique_ptr<ScaledHistogram1D> DCSHistogram<class_ConversionToDCS>::make_1D_vs_Theta_deg(const size_t DimSize_R, const double DimDelta_R) {
	try {
		...
		//引数をポイント数にした関数を定義する
		auto f_pnt2Theta_deg = [=](const double dPnt)->double {return class_ConversionToDCS::Theta_deg(Pnt2X_Backward(dPnt, DimDelta_R)); };
		//auto df_pnt2Theta_deg = [=](const double dPnt)->double {return gv_rad2deg * class_ConversionToDCS::dThetadR(Pnt2X_Backward(dPnt, DimDelta_R)) * DiffOf_Pnt2X_Backward(dPnt, DimDelta_R); };
...
std::unique_ptr<ScaledHistogram1D> DCSHistogram<class_ConversionToDCS>::make_1D_vs_R(const size_t DimSize_R, const double DimDelta_R) {
	try {
		...
		//bin幅で関数を特殊化
		auto f_pnt2R = [=](const double dPnt)->double {return Pnt2X_Backward(dPnt, DimDelta_R); };
		//auto df_pnt2R=[=](const double dPnt)->double {return DiffOf_Pnt2X_Backward(dPnt, DimDelta_R); };
...
template<class class_ConversionToDCS>
std::unique_ptr<ScaledHistogram2D> DCSHistogram<class_ConversionToDCS>::make_2D_vs_Theta_deg_VarPhi_deg(const size_t DimSize_R, const double DimDelta_R, const size_t DimSize_Phi_deg) {
	try {
		
		const double DimDelta_Phi_deg = 360.0 / static_cast<double>(DimSize_Phi_deg);
		
		//ヒストグラム横軸の基本情報
		auto pinf_Theta_deg = make_PointInformation(DimSize_R, PointInformation::Difference::Backward);
		auto pinf_VarPhi_deg = make_PointInformation(DimSize_Phi_deg, PointInformation::Difference::Central0);

		//ポイントから散乱角度を作る
		auto f_pnt2Theta_deg = [=](const double dPnt)->double {
			return class_ConversionToDCS::Theta_deg(Pnt2X_Backward(dPnt, DimDelta_R));
		};
		auto f_pnt2VarPhi_deg = [=](const double dPnt)->double {
			return class_ConversionToDCS::VarPhi_deg(Pnt2X_Central0(dPnt, DimDelta_Phi_deg));
		};
		
		//ヒストグラムを作る
		return make_ScaledHistogram2D(pinf_Theta_deg, pinf_VarPhi_deg, f_pnt2Theta_deg, f_pnt2VarPhi_deg);
	}
	catch (std::invalid_argument&) {
		throw;
	}
...
template<class class_ConversionToDCS>
std::unique_ptr<ScaledHistogram2D> DCSHistogram<class_ConversionToDCS>::make_2D_vs_R_Phi_deg(const size_t DimSize_R, const double DimDelta_R, const size_t DimSize_Phi_deg) {
	try {
		...
		auto pinf_Phi_deg = make_PointInformation(DimSize_Phi_deg, PointInformation::Difference::Central0);

		//ポイントから検出器上の極座標を作る
		auto f_pnt2R = [=](const double dPnt)->double {return Pnt2X_Backward(dPnt, DimDelta_R); };
		auto f_pnt2Phi_deg = [=](const double dPnt)->double {return Pnt2X_Central0(dPnt, DimDelta_Phi_deg); };

		//ヒストグラムを作る
		return make_ScaledHistogram2D(pinf_R, pinf_Phi_deg, f_pnt2R, f_pnt2Phi_deg);
	}
	catch (std::invalid_argument&) {
		throw;
	}
...
public:
	...
	virtual std::vector<double> ErrorOfCalibrated_Hist_Theta_deg(ScaledHistogram1D& Hist_Theta_deg) {
		try {
			...
			const double del_e = gv_del_e;//中心ほどスペースチャージが集中して，BBの裏側へ回り込む信号が増える？
...
template<class class_ConversionToDCS, class class_TheoreticalAtomicDCS_Elastic32, class class_SourceEvents>
std::unique_ptr<DetectorSensitivityDistribution2D<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32, class_SourceEvents>> DetectorSensitivityDistribution2D<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32, class_SourceEvents>::make(const size_t DimSize_R, const double DimDelta_R, const size_t DimSize_Phi_deg) {
	try {
		
		const double DimDelta_Phi_deg = 360.0 / static_cast<double>(DimSize_Phi_deg);//[0, 360)
		
		//カウントデータを累積する
		std::unique_ptr<ScaledHistogram2D> hist_R_Phi_deg = DCSHistogram<class_ConversionToDCS>::make_2D_vs_R_Phi_deg(DimSize_R, DimDelta_R, DimSize_Phi_deg);
		std::unique_ptr<ScaledHistogram2D> hist_Theta_deg_VarPhi_deg = DCSHistogram<class_ConversionToDCS>::make_2D_vs_Theta_deg_VarPhi_deg(DimSize_R, DimDelta_R, DimSize_Phi_deg);
		...
		for (const auto& v : class_SourceEvents::r_phi) {//イベントデータ
			...
			if (cr == 0) {
				...
				//加算
				if (sinfo_R.IsInRange(vr)&&sinfo_Phi_deg.IsInRange(vphi)) {
					hist_R_Phi_deg->AddValue(vr, vphi);
					hist_Theta_deg_VarPhi_deg->AddValue(class_ConversionToDCS::Theta_deg(vr), class_ConversionToDCS::VarPhi_deg_0_360(vphi));
				}
		...
		//式から計算される
		std::unique_ptr<std::vector<std::vector<double>>> Curve_DCS_vs_R_Phi_deg = DCSTheoretical<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32>::make_2D_vs_R_Phi_deg(hist_R_Phi_deg->ScaleInfo0(), hist_R_Phi_deg->ScaleInfo1());
		std::unique_ptr<std::vector<std::vector<double>>> Curve_DCS_vs_Theta_deg_VarPhi_deg = DCSTheoretical<class_ConversionToDCS, class_TheoreticalAtomicDCS_Elastic32>::make_2D_vs_Theta_deg_VarPhi_deg(hist_Theta_deg_VarPhi_deg->ScaleInfo0(), hist_Theta_deg_VarPhi_deg->ScaleInfo1());
		...
		for (size_t i = 0; i < DimSize_R; i++) {
			...
			v_tvp.reserve(DimSize_Phi_deg);
			v_rp.reserve(DimSize_Phi_deg);

			auto v_tvp_err = std::vector<double>();
			auto v_rp_err = std::vector<double>();
			v_tvp_err.reserve(DimSize_Phi_deg);
			v_rp_err.reserve(DimSize_Phi_deg);

			for (size_t j = 0; j < DimSize_Phi_deg; j++) {
...
template class  DetectorSensitivityDistribution1D<Conv_1000_1000, Elastic32_He_1000eV, SourceEvents_DetectorSensitivity>;


template class  DetectorSensitivityDistribution2D<Conv_1000_1000, Elastic32_He_1000eV, SourceEvents_DetectorSensitivity>;
